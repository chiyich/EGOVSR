...
class RandomVideoBlur:
    ...
    def _apply_random_compression(self, imgs):
        frame_num = random.choices(self.params['frame'],
                               self.params['frame_prob'])[0]
        if frame_num==1:
            return imgs
        outputs = []
        for img in imgs:
            outputs.append(img.copy())
        scale = (1-random.random()*0.2)/frame_num
        for i in range(frame_num//2, len(imgs)//2-frame_num//2): #begin frame
            if np.random.uniform() > self.params['prob']:
                continue
            target = i
            outputs[target] *= 1-scale*frame_num
            for j in range(i-frame_num//2,i+frame_num//2+1):
                outputs[target] += imgs[j]*scale
            outputs[target] = np.clip(outputs[target],0,1)
        for i in range(len(imgs)//2+frame_num//2, len(imgs)-frame_num//2): #begin frame
            if np.random.uniform() > self.params['prob']:
                continue
            target = i
            outputs[target] *= 1-scale*frame_num
        ...
        #         import time
        #         a = str(time.time())
        #         cv2.imwrite("/home/chiyc/Data/VSR/mmediting/experiments/realbasicvsr_c64b20_1x30x8_lr5e-5_150k_reds/save/"+str(frame_num)+'.'+str(scale)+"O.jpg", (outputs[i]*255).astype(np.uint8))
        #         cv2.imwrite("/home/chiyc/Data/VSR/mmediting/experiments/realbasicvsr_c64b20_1x30x8_lr5e-5_150k_reds/save/"+str(frame_num)+'.'+str(scale)+"I.jpg", (imgs[i]*255).astype(np.uint8))
        return outputs
...
allowed_degradations = {
    ...
    'RandomVideoBlur': RandomVideoBlur
}
...
class RandomSaveImg:
    ...
    def _apply_random_compression(self, imgs):
        ...
        for i in range(7):
            import time
            a = str(time.time())
            cv2.imwrite("/home/chiyc/Data/VSR/mmediting/experiments/realbasicvsr_c64b20_1x30x8_lr5e-5_150k_reds/save/"+str(a)+"I.jpg", (imgs[i]*255).astype(np.uint8))
        return imgs
