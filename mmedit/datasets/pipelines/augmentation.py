    ...
    def __call__(self, results):
        ...
        if isinstance(results[self.keys[0]],list):
            ...
            for key, out_key in zip(self.keys, self.output_keys):
                temp_keys = results[key]
                results[out_key] = []
                for imgs in temp_keys:
                    result_now = self._resize(imgs)
                    if len(result_now.shape) == 2:
                        result_now = np.expand_dims(result_now, axis=2)
                    results[out_key].append(result_now)
